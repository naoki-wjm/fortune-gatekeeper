...
describe("アスペクトの判定", () => {
  ...
  it("月級の速さでも接近・離反を誤らない（1 日後比較方式の退行防止）", () => {
    // 月（13°/日）が 359.5° から 0° のネイタルへ接近中（コンジャンクション、オーブ 0.5°）。
    // 旧方式だと 1 日後には 12.5° 通過後なので「離反」に化けていた
    expect(isApplying(0, 359.5, 0, 13, 0)).toBe(true);
    // 通過して 0.5° 先へ離れていく月は離反のまま
    expect(isApplying(0, 0.5, 0, 13, 0)).toBe(false);
    // 逆行の月（負の速度）が 0.5° 先から戻ってくるなら接近
    expect(isApplying(0, 0.5, 0, -13, 0)).toBe(true);
    // 太陽級（1°/日）・オーブ 0.4° の接近も正しく「接近」（旧方式の誤判定ケース）
    expect(isApplying(0, 359.6, 0, 1, 0)).toBe(true);
  });
