...
describe("占星術層の initialize / tools/list", () => {
  it("serverInfo はカード層と同じ名前、instructions は別文", async () => {
    ...
    // 体系の数は書かない（占術は増えるので、数を書くとそのたびに嘘になる）
    expect(instructions).not.toContain("三体系");
    expect(instructions).not.toContain("四体系");
    ...
    expect(instructions).toContain("カード・易・ダイス・ジオマンシーもまた別の体系です（上と同じく、合算の根拠にはなりません）");
    // 作者の氏名は書かない
  ...
  it("26 本のツールを返す（占星術層 20 本＋カード層 6 本のスーパーセット）", async () => {
    ...
    expect(names).toEqual([
      // 科ごとの並び（2026-08-25 に歴史順から組み替え）＝出生図の台帳
      "save_chart",
      "list_charts",
      "get_chart",
      "delete_chart",
 ...
 * - 2026-08-25 コンポジットの composite（18 本目）を末尾に追加で更新
 *   （既存 17 本は 1 文字も動かしていない）
...
const FROZEN_ASTRO_TOOLS = [
  ...
  },
  {
    ...
    "description": "登録済みの出生図 2 枚から**コンポジット（中点図）**を組み立てる。2 人の関係そのものを 1 枚の図として見るときのもので、synastry（2 枚の間のアスペクト）とは見ているものが違う。\n採るのは**中点法**——A と B の同じ天体どうしの中点を取る方式で、**ダヴィソン法ではない**（ダヴィソンは 2 人の出生時刻・出生地の中間で図を立て直す別物）。\na / b は**どちらも呼び出した人の台帳の chart_id**（list_charts で確認できる）。c を足すと**三者読み**——A×B の関係図に第三者 C がどう関わるかを見る（c は a / b と同じ ID でもよく、そのときは本人と関係図の重なりを見ることになる）。\n返るのは (1) 中点図の 10 天体（太陽〜冥王星。ノードは扱わない）とサイン・度数、(2) ASC / MC と 12 ハウスカスプ、(3) 中点図の中のアスペクト（メジャー5種＝合・セクスタイル・スクエア・トライン・オポジション、既定オーブ 5°＝orb で変えられる。10 天体＋ASC/MC）、(4) c があれば中点図 × C の総当たりアスペクトと、互いのハウスに相手の天体がどう入るか（ハウスオーバーレイ）。\n**採った規約は名前で固定して返り値にも書く**（流派で割れるところが多いので、読む側が「この鯖はこの流派」と分かるように）——中点は短い方の弧の真ん中（ぴったり 180° のときだけ A から黄経が増える向きに 90°）/ ASC とカスプは中点 MC を ARMC に直し、**2 人の出生緯度の平均**で立て直す（黄道傾斜は 2 人の出生時刻の中間時点のもの）/ ハウス方式は 2 枚が同じならそれ、違えばプラシーダス（P）/ 出生データを預かっていない古い登録が混ざっているときだけ簡易方式（ASC も 2 枚の ASC の中点・カスプは 30° 等分）に落ちる。\n⚠ **中点図のハウスは参考程度**、というのが通説（中点図には「立てた場所と時刻」が無く、ASC とカスプは 2 人の緯度の平均から作った仮のもの）。天体と天体のアスペクトのほうが芯にある。\nこのツールは解釈をしない——相性の良し悪しも組み合わせの意味もサーバーに載せていないので、読みはあなた自身の知識で。\n出生データそのものは返事に出さない（中点の座標という派生値だけを返し、A / B それぞれの天体の黄経も、出生地の緯度も中間緯度も出さない）。",
    "inputSchema": {
      "type": "object",
      "properties": {
        "a": {
        ...
        "c": {
          "type": "string",
          "description": "三者読みで足す第三者のチャート ID（任意）。中点図 × C のアスペクトとハウスオーバーレイが増える（a / b と同じ ID でもよい）"
        },
        "orb": {
          "type": "number",
          "minimum": 0.5,
          "maximum": 10,
          "description": "アスペクトのオーブ（度）。省略すると 5°（止まった図なので広めに取るのが通例。トランジットの 1° とは別）"
        }
      },
      "required": [
        "a",
  ...
  },
  {
    ...
    "title": "四柱の多者盤面（2〜4 人）",
    "description": "登録済みの出生図 2〜4 枚から四柱推命の命式を立てて横に並べ、**日主・地支・空亡のつながりを表引きで**拾う。\nひとりぶんの命式そのもの（通変星・十二運・蔵干・大運・流年）は four_pillars の持ち場で、こちらが返すのは**人と人のあいだに立つ関係だけ**。\ncharts は**呼び出した人の台帳の chart_id の配列**（2〜4 枚。list_charts で確認できる）。同じ ID を 2 つ入れることはできない。時柱を立てるので、出生データを預かっていない古い登録では引けない（登録し直しを案内する）。\n返るのは (1) 各人の日主（天干・五行・陰陽）と 4 柱の干支・空亡、(2) 全ペアぶん ―― 日主の関係（比和／相生／相剋。どちらがどちらを生む・剋すかつき、天干五合が立てばそれも）と、**一方の各柱 × 他方の各柱の総当たり**で立つ地支の関係（六合・六沖・半合＝三合の 2 支・同一支）、「X の空亡に Y のどの柱の地支が入るか」を双方向で、(3) **3 人以上なら**全員の地支を持ち寄って揃う三合局・方合（誰のどの柱がどの支を出しているかつき。1 人で 3 支そろえている局は「単独で成立」と別枠）と、空亡の有向辺の連鎖（環が閉じていれば「相互」「三すくみ」と名前で）。\n**採った規約は名前で固定して返り値にも書く**（流派で割れるところが多いので、読む側が「この鯖はこの流派」と分かるように）——日界 0 時（23 時台生まれの「夜子時」は採らず、既定の 1 通りだけで並べる）/ 空亡は日柱の旬から取り、相手のどの柱の地支も見る / 半合は三合局の 3 支のうち 2 支（旺支を含まない組も同じ半合として数える）/ **刑・害・破は含めない**（拾い忘れではなく採らないという意味で、conventions の excluded に名前を書く）。\n**このツールは解釈をしない**——関係の名前を並べるだけで、**点数化も多数決もしない**（合の数を数えて相性の点にしたり、凶の札を足し合わせたりはサーバーの持ち場ではない）。読みはあなた自身の知識で。どれだけ体系を横断し、それらが全て同じ結果を示したとて、合算の根拠にはならない（面白がるのは自由）。\n出生データそのものは返事に出さない（命式＝干支のような派生値だけを返す）。",
    "inputSchema": {
      "type": "object",
      "properties": {
        "charts": {
          ...
          "minItems": 2,
          "maxItems": 4,
          "description": "横に並べるチャート ID の配列（2〜4 枚。list_charts で確認できる）。同じ ID を 2 つ入れることはできない"
        }
      },
      "required": [
        "charts"
