...
Claude が会話の流れから直接「占いを引ける」ようにする MCP サーバー。Cloudflare Workers の上で動く、一本の直通電話。公開の**カード層**（カード占い6デッキ・易占（周易）・アストロダイス・ジオマンシー）と、鍵つきの**占星術層**（ホロスコープ計算・数秘術）が入っています。
...
fortune-gatekeeper/
  ...
  src/astro/astro-mcp.ts … 占星術層の MCP ハンドラ（鍵つき入口・ツール12本）
