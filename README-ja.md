...
アスペクトはメジャー5種（合・セクスタイル・スクエア・トライン・オポジション）・オーブ 1°。`get_chart` の出生図の中のアスペクトだけは既定 5°（astro-viewer の natal オーブと同じ。`orb` で 0.5〜10° に変えられる）。ハウス方式は P（プラシーダス）/ K（コッホ）/ W（ホールサイン）/ E（イコール）。天体は太陽から冥王星までの10個＋ノース ノード（真）の11個です。
