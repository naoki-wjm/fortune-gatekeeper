...
export async function fetchUpstreamAuthToken(params: {
	...
	if (!response.ok) {
		// 上流の本文はブラウザへ反射しない（認可コード等が含まれうる）。参照IDと status だけ記録する。
		const ref = crypto.randomUUID();
		console.error(`upstream token exchange failed [${ref}] status=${response.status}`);
		return [
			null,
			null,
			new Response(`Failed to exchange code for token (ref ${ref})`, { status: 502 }),
		];
	}
